...
r = session.get("http://challs.dvc.tf:6002/home", params={"value": "Goooo"})
output = r.text

ar = re.findall("input.*", output)
ar.remove(ar[-1])
...
for i in ar:
    i = i.replace('input class="case', '')[2:].replace('" type = "text" name = ', '').replace(' />', '')[2:].replace('value = ', '').replace(' disabled/>', '').replace(" ", "")
    if (i == ""):
        row.append(0)
    else: 
        row.append(int(i))
...
for row in board:
    for i in row:
        payload.append((str(c), str(i)))
        c+=1
x = session.post("http://challs.dvc.tf:6002/flag", data=payload) 
